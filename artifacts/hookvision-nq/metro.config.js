...
// The Replit proxy routes /hookvision-nq/* to port 25352 and strips the prefix.
// Strategy:
//   - Root request "/" → rewrite to "/hookvision-nq/" so Metro includes
//     transform.baseUrl=%2Fhookvision-nq in the generated HTML bundle URL.
...
config.server.rewriteRequestUrl = (url) => {
  ...
  // Metro generates the HTML with transform.baseUrl=%2Fhookvision-nq.
  ...
  return (req, res, next) => {
    ...
    res.end = (chunk, ...args) => {
      ...
      if (buf.includes("<html") || buf.includes("<!DOCTYPE")) {
        // 1. Add /hookvision-nq prefix to root-relative asset URLs so the
        //    Replit proxy routes them to THIS Metro server (port 25352).
        buf = buf
