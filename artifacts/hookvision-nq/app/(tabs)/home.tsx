...
// ─── Brisbane/QLD local time helper ─────────────────────────────────────────────────

function getQldTime() {
  const now = new Date();
  const hour = parseInt(
    now.toLocaleString("en-AU", { hour: "numeric", hour12: false, timeZone: "Australia/Brisbane" }),
    10
  );
...
export default function HomeScreen() {
  ...
  return (
    ...
    >
      ...
      {conds?.weather && (
        <>
          <Text style={[S.sectionHead, { color: colors.mutedForeground }]}>NQ LIVE WEATHER</Text>
