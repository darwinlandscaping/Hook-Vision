...
function getNQSeason(m: number): { name: string; waterTemp: string; short: string } {
  if (m >= 5 && m <= 9)  return { name: "Gulf Dry Season",   waterTemp: "23–26°C", short: "Dry" };
  if (m === 10 || m === 11) return { name: "Build-Up (QLD)", waterTemp: "27–30°C", short: "Build-Up" };
  return { name: "Gulf Wet Season", waterTemp: "28–32°C", short: "Wet" };
}
...
const SPOT_POOL = [
  { name: "Karumba Point Beach",     river: "Norman River",      region: "Gulf of Carpentaria", lure: "Surface walker 100mm",  species: "Queenfish + Barra 70cm+", emoji: "🏖️", bestMoon: ["spring"], bestSeason: ["Dry","Build-Up"], tidal: true,  baseScore: 90 },
  { name: "Norman River Cut Bank",   river: "Norman River",      region: "Gulf of Carpentaria", lure: "Hard-body minnow 90mm", species: "Barra 65–100cm+",         emoji: "🌊", bestMoon: ["spring"], bestSeason: ["Dry","Build-Up"], tidal: true,  baseScore: 88 },
  { name: "Normanton Wharf Hole",    river: "Norman River",      region: "Gulf of Carpentaria", lure: "Heavy vibe 80g",        species: "Barra + Black Jewfish",   emoji: "⚓", bestMoon: ["spring"], bestSeason: ["Dry"],            tidal: true,  baseScore: 85 },
  { name: "Mitchell River Mouth",    river: "Mitchell River",    region: "Gulf of Carpentaria", lure: "Surface walker 120mm",  species: "Barra 70–100cm+",         emoji: "🌄", bestMoon: ["spring"], bestSeason: ["Wet","Build-Up"], tidal: true,  baseScore: 86 },
  { name: "Weipa Causeway",          river: "Embley River",      region: "Western Cape York",   lure: "GT popper 160mm",       species: "GT + Barra + Jack",       emoji: "🎯", bestMoon: ["spring"], bestSeason: ["Dry"],            tidal: true,  baseScore: 87 },
  { name: "Flinders River Mouth",    river: "Flinders River",    region: "Gulf of Carpentaria", lure: "Metal slice 40g",       species: "Barra + Queenfish",       emoji: "🏞️", bestMoon: ["any"],    bestSeason: ["Dry","Build-Up"], tidal: true,  baseScore: 78 },
  { name: "Gilbert River Mouth",     river: "Gilbert River",     region: "Gulf of Carpentaria", lure: "Surface walker 80mm",   species: "Barra 60–95cm",           emoji: "🌿", bestMoon: ["any"],    bestSeason: ["Build-Up","Wet"], tidal: true,  baseScore: 80 },
  { name: "Albert River Burketown",  river: "Albert River",      region: "Southern Gulf",       lure: "Surface walk-the-dog",  species: "Barra 70–100cm+",         emoji: "🪨", bestMoon: ["spring"], bestSeason: ["Dry"],            tidal: true,  baseScore: 82 },
  { name: "Endeavour River Bend",    river: "Endeavour River",   region: "Far North QLD",       lure: "Deep-diver 80mm",       species: "Barra + Jack",            emoji: "🐊", bestMoon: ["any"],    bestSeason: ["Dry"],            tidal: true,  baseScore: 72 },
  { name: "Karumba Bay Mouth Bar",   river: "Norman/Bynoe",      region: "Gulf of Carpentaria", lure: "Rattling hard-body",    species: "Queenfish + Threadfin",   emoji: "🐟", bestMoon: ["any"],    bestSeason: ["Dry","Build-Up"], tidal: true,  baseScore: 74 },
  { name: "Archer River Mouth",      river: "Archer River",      region: "Western Cape York",   lure: "Hardbody minnow 90mm",  species: "Barra + Jack",            emoji: "🎣", bestMoon: ["any"],    bestSeason: ["Dry"],            tidal: true,  baseScore: 70 },
...
const BARRA_COMPS = [
  {
    name: "Karumba Barra Classic",
    host: "Karumba Fishing Club",
    emoji: "🏆",
    color: "#ffd700",
    description: "The Gulf Country's premier barramundi competition. Held out of Karumba on the Norman River — the heartland of Queensland barra fishing. Open, Women's, and Junior divisions.",
    dates: "August each year (peak Dry Season)",
    website: "https://www.karumbafishingclub.com.au",
    highlight: "Gulf Country's biggest annual barra comp",
  },
  {
    name: "Weipa GT & Barra Tournament",
    host: "Weipa Fishing Club",
    emoji: "🎯",
    color: "#ff2200",
    description: "Weipa's annual tournament covering both barramundi in the Embley River estuary and giant trevally on the causeway and offshore reefs. World-class sport fishing.",
    dates: "June–July (Dry Season)",
    website: "https://www.weipafishingclub.com.au",
    highlight: "GT + Barra combined — epic Cape York fishing",
  },
  {
    name: "Cairns Offshore Classic",
    host: "Cairns Sport Fishing Club",
    emoji: "🌅",
    color: "#00a8ff",
    description: "Cairns's premier offshore fishing tournament covering coral trout, Spanish mackerel, GT, and pelagic species. Based at the Marlin Marina with access to the outer GBR.",
    dates: "September–October (pre-marlin season)",
    website: "https://www.cairnssportfishing.com.au",
    highlight: "GBR reef fishing — coral trout, macks, GTs",
  },
  {
    name: "Burketown Remote Barra Classic",
    host: "Burketown Fishing Association",
    emoji: "🌊",
    color: "#00d4aa",
    description: "Deep in the southern Gulf, the Burketown Barra Classic on the Albert River is one of Queensland's most remote fishing comps — barra to 1 m+ in pristine isolation.",
    dates: "July (Dry Season)",
    website: "https://www.burketown.qld.gov.au",
    highlight: "Most remote QLD barra comp — true Gulf Country",
  },
];
...
const BARRA_FACTS = [
  ...
  { emoji: "🌧️", fact: "In Queensland, barramundi spawn during the Wet Season (Nov–Mar) in Gulf tidal estuaries like the Norman, Mitchell, and Flinders rivers." },
  ...
  { emoji: "👁️", fact: "Barramundi have exceptional low-light vision, which is why dawn/dusk fishing on Gulf river edges is so productive." },
  { emoji: "🌊", fact: "Queensland Gulf barra can move 300km+ during their lifetime — from freshwater billabongs to tidal river mouths and the Gulf of Carpentaria." },
  { emoji: "🦈", fact: "A big barra will eat anything that fits in its mouth, including small birds, lizards, and even other barra." },
  { emoji: "🏆", fact: "Queensland's bag limit is 5 barra per person with a 58 cm minimum — helping protect the trophy-sized females that drive the spawn." },
      ...
      <View style={styles.mdfGradient}>
        ...
        <View style={styles.mdfPrizes}>
          <View style={[styles.mdfPrize, { borderColor: "#ffd70066" }]}>
            <Text style={styles.mdfPrizeAmt}>OPEN DIVISION</Text>
            <Text style={styles.mdfPrizeLabel}>TROPHY BARRA</Text>
          </View>
          <View style={[styles.mdfPrize, { borderColor: "#ffd70044" }]}>
            <Text style={styles.mdfPrizeAmt}>WOMEN'S</Text>
            <Text style={styles.mdfPrizeLabel}>TOP CATEGORY</Text>
          </View>
          <View style={[styles.mdfPrize, { borderColor: "#ffd70033" }]}>
            <Text style={styles.mdfPrizeAmt}>JUNIOR</Text>
            <Text style={styles.mdfPrizeLabel}>& KAYAK</Text>
          </View>
        </View>

        <Text style={styles.mdfHow}>
          Queensland's premier Gulf barra tournament. Fish the Norman River from Karumba for trophy barra in the August Dry Season. Open, Women's, and Junior categories. Register online.
        </Text>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.8}
        >
          <Text style={styles.mdfBtnText}>Learn More & Register →</Text>
        </TouchableOpacity>
      </View>
    </Animated.View>
  );
...
export default function BarraScreen() {
  ...
  const season  = getNQSeason(month);
  const darwinHour = parseInt(now.toLocaleString("en-AU", { hour: "numeric", hour12: false, timeZone: "Australia/Brisbane" }), 10);
  const isGoldHour = (darwinHour >= 5 && darwinHour <= 8) || (darwinHour >= 16 && darwinHour <= 20);
  ...
  useAutoNarrate(() => {
    ...
    return `NQ Barra Nation. Your complete Queensland barramundi hub. Top hot spot right now: ${top.name} on the ${top.river}. ${moon.emoji} ${moon.name}, ${season.name}. ${isGoldHour ? "Golden hour — get out there now!" : "Fish are feeding. Check the hot spots."}`;
  });
  ...
  return (
    ...
    >
      ...
      <NarratorButton pageType="nq barra nation" content={`NQ Barra Nation — North Queensland barramundi hub. ${season.name}, ${moon.name}. Top hot spot right now: ${hotSpots[0].name} on the ${hotSpots[0].river}. Karumba Barra Classic: next event in August. Upcoming comps: Weipa GT & Barra Tournament, Burketown Remote Barra Classic. Fun fact: ${BARRA_FACTS[0].fact}`} />
    </ScrollView>
  );
}
