...
export default function HomeScreen() {
  ...
  const analyzeImage = useCallback(async () => {
    ...
    const flashPromise = Platform.OS === "web"
      ? null
      : fetch(`${apiBaseUrl}${isLiveSonar ? "/api/live-sonar-analyze/flash" : "/api/analyze/flash"}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ imageBase64 }),
    ...
    try {
      ...
      if (response.body) {
        ...
        try {
          ...
          // Network dropped mid-stream — if we have enough data try to parse what arrived,
          // otherwise surface a clear retry message
          if (accumulated.length < 20) {
            throw new Error("Connection dropped — please try again.");
          }
          // else fall through: accumulated may have enough JSON to parse
        } finally {
      ...
      // ── Auto-save to Brain library (fire-and-forget) ──────────────────────
      {
        ...
        fetch(`${apiBaseUrl}/api/brain/sonar`, {
          method:  "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if ((data.confidence ?? 0) >= 75 && (data.fishCount ?? 0) > 0 && imageBase64) {
        const scanDate    = new Date().toLocaleDateString("en-AU", { day: "2-digit", month: "short", year: "numeric" });
        fetch(`${apiBaseUrl}/api/sonar-brain/submit`, {
          method:  "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            imageBase64,
      ...
      try {
        ...
        fetch(`${apiBaseUrl}/api/community/report`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            species: data.species,
              ...
              <>
                <View style={styles.sonarBrainRow}>
                  ...
                  <Text style={[styles.sonarBrainVerdict, {
                    ...
                    {(() => {
                      // If main analysis already confirmed a non-Barra species, label arches with that species
                      ...
                      const archLabel = (mainHasSpecies && !mainIsBarra)
                        ? mainSpecies!.toUpperCase()
                        : "BARRA";
  ...
  return (
    <ScrollView
      ...
      showsVerticalScrollIndicator={false}
    >
      <HVHeader subtitle="WA/Kimberley Fishing" />
      ...
      >
        ...
        <View style={styles.heroInner}>
          ...
          <Text style={[styles.heroDesc, { color: colors.mutedForeground }]}>
            Try AI analysis on real WA sonar screenshots — barra arches, structure maps,
