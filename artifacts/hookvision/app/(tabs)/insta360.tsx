...
export default function Insta360Screen() {
  ...
  useEffect(() => {
    ...
    const ip = activeBaseUrl
      ...
      : "192.168.42.1";
    // Send _startPreview command — activates the low-latency JPEG frame endpoint
    // at http://CAMERA_IP:20000/preview.jpg on X4/X3/ONE RS/Go 3.
    // Fails silently on older models; frame polling still works if the camera
    // has preview active from another connection or supports auto-preview.
    ...
    // Tick every 800 ms — each tick forces <Image> to reload the preview frame
      ...
      >
        ...
        {isConnected && camType === "insta360" && (
          <View style={[styles.card, { gap: 12, borderColor: C.teal + "55", borderWidth: 1.5 }]}>
            ...
            {/* Live frame — polls http://CAMERA_IP:20000/preview.jpg every 800 ms */}
            ...
            {/* Full-res snapshot result (taken via Full Shot button) */}
            {previewUri && (
              <View style={{ gap: 6 }}>
